...
def solve():
    ...
    accessible = np.ones(matrix.shape, matrix.dtype)
    result = 0
    while accessible.sum() > 0: # some rolls can be accessed
        accessible = convolution(matrix) # find accessible ones
        result += np.sum(accessible) # add this round sum to the result
        matrix = matrix - accessible # remove them
    print(result)
